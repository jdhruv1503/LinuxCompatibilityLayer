...
5. **Debug Logging for Network Verification**: Since pcap isn't available, `sdkconfig.defaults` enables DEBUG-level logging. The firmware sets `esp_log_level_set("shim_socket", ESP_LOG_DEBUG)` to log socket operations. Filter output with: `grep -E "(socket|send|connect|dup2)"`. This is safe from infinite loops because ESP_LOG writes directly to UART, not through shim_write.

### Stdout Redirection (dup2)
...
2. **Global Socket Variable Approach**: For stdout redirection:
   - Don't modify the FD table
   - Store target socket in a global variable
   - Set a flag indicating stdout is "redirected"
   - In `shim_write()`, check the flag and dual-route writes
...
| Tool | Purpose |
|------|---------|
| `tools/build_and_run.py` | **Primary tool** - Full build & simulation workflow |
| `tools/run_build.bat` | Wrapper for `idf.py` (handles MSYSTEM) |
| `tools/export_symbols.py` | Generate symbol table from ELF |
| `tools/build_guest_app.bat` | Compile guest ELF applications |
